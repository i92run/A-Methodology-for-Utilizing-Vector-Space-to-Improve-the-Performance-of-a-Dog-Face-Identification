...
def ArcFace_image_generator(filenames, labels, batch_size=63, use_aug=True, datagen=datagen):
    """
    ...
    while True:
        ...
        X = load_images(filenames[batch_idx])
        Y = labels[batch_idx]

        if use_aug:
            X = apply_transform(X, datagen)
        yield ([X, Y], Y)

def ArcFace_triplet_image_generator(filenames, labels, batch_size=63, use_aug=True, datagen=datagen):
    """
    Training generator for soft triplets.
    """
    while True:
...
def define_adaptive_hard_triplets_batch(filenames,labels,predict,nbof_triplet=21*3, use_neg=True, use_pos=True):
    ...
    triplets = []
    y_triplets = np.empty(nbof_triplet)
    pred_triplets1 = np.empty((nbof_triplet, predict[0].shape[-1]))
    pred_triplets2 = np.empty((nbof_triplet, predict[1].shape[-1]))
    ...
    for i in range(0,nbof_triplet,3):
        ...
        # Computes the distance between the chosen image and the rest of the class
        if use_pos:
            dist_class = np.sum(np.square(predict[0][keep]-predict[0][keep][idx_image1]),axis=-1)
        ...
        triplets += [keep_filenames[idx_image1]]
        y_triplets[i] = keep_labels[idx_image1]
        pred_triplets1[i] = predict[0][keep][idx_image1]
        pred_triplets2[i] = predict[1][keep][idx_image1]
        triplets += [keep_filenames[idx_image2]]
        y_triplets[i+1] = keep_labels[idx_image2]
        pred_triplets1[i+1] = predict[0][keep][idx_image2]
        pred_triplets2[i + 1] = predict[1][keep][idx_image2]

        # Computes the distance between the chosen image and the rest of the other classes
        not_keep = np.logical_not(keep)

        if use_neg:
            dist_other = np.sum(np.square(predict[0][not_keep]-predict[0][keep][idx_image1]),axis=-1)
            idx_image3 = np.argmin(dist_other)
        else:
            idx_image3 = np.random.randint(len(filenames[not_keep]))
        ...
        triplets += [filenames[not_keep][idx_image3]]
        y_triplets[i+2] = labels[not_keep][idx_image3]
        pred_triplets1[i+2] = predict[0][not_keep][idx_image3]
        pred_triplets2[i + 2] = predict[1][not_keep][idx_image3]

    return np.array(triplets), y_triplets, pred_triplets1, pred_triplets2

def load_images(filenames: object):
    """
    ...
    for i,f in enumerate(filenames):
        if 'jpg' in f:
            temp = skimage.io.imread(f).reshape(h,w,c)
            images[i] = temp / 255.0
    return images

def load_images_multi(filenames: object, color=True):
    """
    Use scikit-image library to load the pictures from files to numpy array.
    """
    if color:
...
def image_generator(filenames, labels, batch_size=63, use_aug=True, datagen=datagen):
    ...
    while True:
        ...
        if use_aug:
            i_triplet = apply_transform(i_triplet, datagen)
        yield (i_triplet, [y_triplet, y_triplet])

def image_generator_multi(filenames, labels, batch_size=63, use_aug=True, datagen=datagen, color1=True, color2=True):
    """
    Training generator for soft triplets.
    """
    while True:
        f_triplet, y_triplet = define_triplets_batch(filenames, labels, batch_size)
        if not color1:
            f_triplet1 = [i.replace('rgb', 'grayscale') for i in f_triplet]
        else:
            f_triplet1 = f_triplet
        if not color2:
            f_triplet2 = [i.replace('rgb', 'grayscale') for i in f_triplet]
        else:
            f_triplet2 = f_triplet

        i_triplet1 = load_images_multi(f_triplet1, color1)
        i_triplet2 = load_images_multi(f_triplet2, color2)
        if use_aug:
            i_triplet1 = apply_transform(i_triplet1, datagen)
            i_triplet2 = apply_transform(i_triplet2, datagen)
        yield ([i_triplet1, i_triplet2], y_triplet)
...
def predict_generator_multi(filenames, batch_size=32, color1=True, color2=True):
    """
    Prediction generator.
    """
    for i in range(0,len(filenames), batch_size):
        if not color1:
            images_batch1 = load_images_multi([j.replace('rgb', 'grayscale') for j in filenames[i:i + batch_size]],color1)
        else:
            images_batch1 = load_images_multi(filenames[i:i + batch_size],color1)

        if not color2:
            images_batch2 = load_images_multi([j.replace('rgb', 'grayscale') for j in filenames[i:i + batch_size]],color2)
        else:
            images_batch2 = load_images_multi(filenames[i:i + batch_size],color2)

        yield [images_batch1, images_batch2]
...
class online_adaptive_hard_image_generators:
    def __init__(self):
        self.loss = 0
        self.hard_triplet_ratio = 0
        self.nbof_hard_triplets = 0
    def online_adaptive_hard_image_generator(self,
        ...
        while True:
            ...
            subclasses = np.random.choice(classes,size=int(nbof_subclasses*self.hard_triplet_ratio)+2,replace=False)
            ...
            f_triplet_hard, y_triplet_hard, predict_hard1, predict_hard2 = define_adaptive_hard_triplets_batch(subfilenames, sublabels, predict, self.nbof_hard_triplets*3, use_neg=True, use_pos=True)
            f_triplet_soft, y_triplet_soft, predict_soft1, predict_soft2 = define_adaptive_hard_triplets_batch(subfilenames, sublabels, predict, batch_size-self.nbof_hard_triplets*3, use_neg=False, use_pos=False)

            f_triplet = np.append(f_triplet_hard,f_triplet_soft)
            y_triplet = np.append(y_triplet_hard,y_triplet_soft)

            predict1 = np.append(predict_hard1, predict_soft1, axis=0)
            predict2 = np.append(predict_hard2, predict_soft2, axis=0)

            # Proportion of hard triplets in the generated batch
            #hard_triplet_ratio = max(0,1.2/(1+np.exp(-10*acc+5.3))-0.19)
            self.hard_triplet_ratio = np.exp(-self.loss * 10 / batch_size)

            if isnan(self.hard_triplet_ratio):
                self.hard_triplet_ratio = 0
            self.nbof_hard_triplets = int(batch_size//3 * self.hard_triplet_ratio)
...
def online_adaptive_hard_image_generator_multi(
        filenames,  # Absolute path of the images
        labels,  # Labels of the images
        model,  # A keras model
        loss,  # Current loss of the model
        ...
        datagen=datagen,
        color1=True,
        color2=True):  # Data augmentation parameter
    """
    Generator to select online hard triplets for training.
    Include an adaptive control on the number of hard triplets included during the training.
    """
    ...
    while True:
        ...
        sublabels = labels[keep_classes]
        # print(sublabels)
        # print(int(np.ceil(len(subfilenames) / 32)))
        # print(predict_generator_multi(subfilenames, 32, color1=color1, color2=color2))
        # aa = predict_generator_multi(subfilenames, 32, color1=color1, color2=color2)
        # bb = int(np.ceil(len(subfilenames) / 32))
        # print(model)
        # print(model.predict_generator(aa, steps=bb))
        predict = model.predict_generator(predict_generator_multi(subfilenames, 32, color1=color1, color2=color2),
                                          steps=int(np.ceil(len(subfilenames) / 32)))
        ...
        f_triplet_soft, y_triplet_soft, predict_soft = define_adaptive_hard_triplets_batch(subfilenames, sublabels,
                                                                                           predict,
