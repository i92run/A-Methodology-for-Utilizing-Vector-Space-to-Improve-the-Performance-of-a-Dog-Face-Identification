...
from online_training_ArcFace_Stage_1 import *

#----------------------------------------------------------------------------
NET_NAME    = 'ArcFace_Stage_1'
VERSION_NAME = 'v3'
PATH = './data/Dog FaceNet DB1/more_than_5_rgb/'
PATH_LOG = './logs/DogFaceNet_ArcFace_Stage_1/' + VERSION_NAME +'/'
PATH_MODEL = './models/DogFaceNet_ArcFace_Stage_1/' + VERSION_NAME +'/'
...
Vector_Length_weight = K.variable(1.0)
...
def TripletArcLoss():
    ...
    def triplet_arc_loss(y_true,y_pred):
        ...
        return K.sum(tf.nn.relu(theta_ap - theta_an + alpha)) * ArcFace_weight
...
def VectorLengthLoss():
    """vector length loss"""
    def vector_length_loss(y_true,y_pred):
        a = y_pred[0::3]
        p = y_pred[1::3]
        n = y_pred[2::3]
        a1 = K.sum(K.square(a), -1)
        p1 = K.sum(K.square(p), -1)
        a2 = K.sqrt(a1)
        p2 = K.sqrt(p1)
        L = K.minimum(a2, p2)
        z = K.abs(a2 - p2)
        y = (K.log(L + 1) * z) + (1 / (L * (L + z))) - beta
        return K.sum(tf.nn.relu(y)) * Vector_Length_weight
    return vector_length_loss

def VectorLengthAcc():
    """distance based vector acc"""
    def vector_length_acc(y_true, y_pred):
        a = y_pred[0::3]
        p = y_pred[1::3]
        n = y_pred[2::3]
        ap = K.sum(K.square(a - p), -1)
        an = K.sum(K.square(a - n), -1)
        aa = K.sum(K.square(a), -1)
        return K.less(ap + aa * alpha, an)
    return vector_length_acc
...
Arc_embds = Lambda(lambda x: tf.nn.l2_normalize(x,axis=-1), name='ArcFace_embds')(x)
Length_embds = Lambda(lambda x: x, name='Vector_Length_embds')(x)

ArcFace_model = tf.keras.Model(inputs, [Arc_embds, Length_embds])
ArcFace_model.compile(loss={'ArcFace_embds':TripletArcLoss(), 'Vector_Length_embds':VectorLengthLoss()},
                      optimizer=tf.keras.optimizers.Adam(1e-4),
                      metrics={'ArcFace_embds':TripletArcAcc(), 'Vector_Length_embds':VectorLengthAcc()})
print('Done.')

model_tensorboard = tf.keras.callbacks.TensorBoard(log_dir=PATH_LOG)
model_checkpoint_callback1 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'triplet_arc_acc_{val_ArcFace_embds_triplet_arc_acc:.2f}.h5',
    save_weights_only=False,
    monitor='val_ArcFace_embds_triplet_arc_acc',
    mode='max',
    save_best_only=True)

model_checkpoint_callback2 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'triplet_arc_loss_{val_ArcFace_embds_loss:.2f}.h5',
    save_weights_only=False,
    monitor='val_ArcFace_embds_loss',
    mode='min',
    save_best_only=True)

model_checkpoint_callback3 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'vector_length_acc_{val_Vector_Length_embds_vector_length_acc:.2f}.h5',
    save_weights_only=False,
    monitor='val_Vector_Length_embds_vector_length_acc',
    mode='max',
    save_best_only=True)

model_checkpoint_callback4 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'vector_length_loss_{val_Vector_Length_embds_loss:.2f}.h5',
    save_weights_only=False,
    monitor='val_Vector_Length_embds_loss',
    mode='min',
    save_best_only=True)

model_checkpoint_callback5 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'LOSS_{val_loss:.2f}.h5',
    save_weights_only=False,
    monitor='val_loss',
    mode='min',
    save_best_only=True)
...
sess = tf.Session()
tf.summary.scalar('hard_triplet_ratio', X1)
tf.summary.scalar('nbof_hard_triplets', X2)
...
histories += [ArcFace_model.fit_generator(
    ...
    verbose=1,
    callbacks=[model_tensorboard, model_checkpoint_callback1, model_checkpoint_callback2, model_checkpoint_callback3,  model_checkpoint_callback4,  model_checkpoint_callback5, CustomCallback()])]
