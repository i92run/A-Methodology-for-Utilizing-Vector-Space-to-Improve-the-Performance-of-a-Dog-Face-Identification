...
#----------------------------------------------------------------------------
NET_NAME    = 'ArcFace_Stage_2'
VERSION_NAME = 'v3'
PATH = './data/Dog FaceNet DB1/more_than_5_rgb/'
PATH_LOG = './logs/DogFaceNet_ArcFace_Stage_2/' + VERSION_NAME +'/'
PATH_MODEL = './models/DogFaceNet_ArcFace_Stage_2/' + VERSION_NAME +'/'
model_name = './models/DogFaceNet_ArcFace_Stage_1/v3/LOSS_0.28.h5'
...
outputs = Lambda(lambda x: x, name='triplet')(x)

ArcFace_model = tf.keras.Model(inputs, outputs)
ArcFace_model.load_weights(model_name)
ArcFace_model.summary()

ArcFace_model.compile(loss=TripletLoss(),
                      optimizer=tf.keras.optimizers.Adam(1e-4),
                      metrics=[TripletAcc()])

model_tensorboard = tf.keras.callbacks.TensorBoard(log_dir=PATH_LOG)
model_checkpoint_callback1 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'triplet_acc_{val_triplet_acc:.2f}.h5',
...
model_checkpoint_callback2 = tf.keras.callbacks.ModelCheckpoint(
    filepath=PATH_MODEL + 'triplet_loss_{val_loss:.2f}.h5',
    save_weights_only=False,
    monitor='val_loss',
    mode='min',
    save_best_only=True)
